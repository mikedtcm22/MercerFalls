---
title: Embers of the Gathering Hollows — Overview
type: moc
domain: Meta
status: canonical
visibility: mixed
tags: [embers, overview, moc, home]
created: 2026-06-14
---

# Embers of the Gathering Hollows — Overview

> Home note for the Mercer Falls campaign vault: what the campaign is, how this vault is organized, and where to start. Living index — each domain below has its own MOC that links out to the atomic notes. Built to mirror the Arachne-style second-brain layout (see [[Vault Setup & Conventions]]) so an LLM (and you) can reach any piece of canon in one or two hops.

## What the campaign is

A **D&D 5e roguelike-megadungeon campaign with *Persona*-style in-town time mechanics**, set in **Mercer Falls, Ohio, summer 1996** — a dying rust-belt town where magic is supposed to be impossible. Kids stumble into a century-old infernal machine beneath the town that **eats agency** (presenting to the world as time-loss), and are saved by **embers** granted by the thing the machine was built to contain. Tonal touchstones: *Buffy*, *Stranger Things* (deliberately diverged), *Persona*. Act 1 runs Levels 1–5, mid-June through the **autumn equinox (Sept 22, 1996)**.

The deepest argument of the campaign: **agency is the answer** — against both cosmic forces that want choice removed. See [[High Concept]] and [[Order and Chaos]].

## How to read this vault

Start here, then follow links. The navigation backbone is **Maps of Content (MOCs)** — one per domain — not folders. `[[Wikilinks]]` resolve by note name regardless of folder, so move notes freely. Every note carries YAML **properties** (`type`, `domain`, `status`, `visibility`, `tags`, `source`) and an `Up:` link to its MOC. DM-secret material is flagged two ways: a `visibility: dm-only` property and an inline `> [!danger] DM-only` callout, so the future MCP connector can filter what a player-facing LLM is allowed to surface. See [[Vault Setup & Conventions]].

## The domains

### [[Lore & Cosmology]] — the hidden truth (mostly DM-only)
The metaphysics under everything: [[The Dungeon]] as an agency-eating machine, [[Order and Chaos]], [[The Entity]] (the contained Chaos that is the PCs' "benevolent" patron), [[The Embers]], [[The Nightmare Engine]], [[The Reality Filter]], [[Hollowing and Agency-Loss]], and [[The 1991 Mill Incident]].

### [[Mercer Falls — Setting]] — the place
Geography and locations, surface and hidden: [[Mercer Falls, Ohio]], [[The Mill]], [[Mercer Lake]], [[The River and the Falls]], [[The Mercer Mansion]], [[The Mercer Gate]], [[The Dungeon's Surface Footprint]], [[The Districts]], [[The Wrongness Layer]], plus establishments ([[Panels]], [[Marco's Diner]], [[The Community Center]], [[Earl's Pawn Shop]], [[The Sinclair Station]], [[The Hungry House]], [[The Fishing Dock]]) and dungeon space ([[The Ember Chamber]], [[The Mill Sector]]).

### [[Cast & NPCs]] — the people
[[Wesley Crane]] (Act 1 antagonist), [[Russ]], the recruits ([[Marcy Hauer]], [[Ricky Vance]], [[Eddie Park]], [[The Fourth Recruit]]), [[Sam]] (bloodline anchor), the adherents ([[Mike and Doug Tinsmith]], [[Phyllis Harrow]]), [[Henry Voss]], [[Elias Mercer]], [[Mrs. Devereaux]], [[Earl]], [[The Antique Dealer]]. Player characters live in [[The Party]].

### [[Factions & Groups]]
[[The Party]], [[Wesley's Recruits]], [[Wesley's Adherents]], [[The Other Party (Rival or Evil)]], [[Mercer Falls Historical Society]], [[The Mercer Gentleman's Club]], [[Hollowed Townspeople]].

### [[Game Systems]] — how it plays
[[The Two Modes of Play]], [[Time Points]], [[The Negotiation System]], [[Dungeon Mechanics]], [[Floor Reset and the Intelligence Gradient]], [[Forensic Delving]], [[Death, TPK, and the Lost Day]], [[The Keystone Tablet]], [[Wesley's Sigils]], [[Gear and Items]].

### [[Plot & Structure]] — the bizarro plot and the shape of Act 1
[[The Bizarro Plot]], [[Wesley's Summer Calendar]], [[The Three Ritual Items]], [[The Equinox Ritual]], [[Aftermath Branches]] (Rival vs Evil fork), [[The Pattern Recognition Arc]], [[The Keystone Theft]], [[Where the PCs Can Disrupt]], [[Cosmology Seeds]], [[The Countdown Structure]].

### [[Episodes]] — the monster-of-the-week catalog
[[Session 0]], [[Session 1 — The Dare]], [[Episode 1 — The Hush]], [[Episode 2 — The Hungry House]], [[Episode 3 — The Click]], [[Episode 4 — The Mirror]], [[Episode 5 — Storm-That-Doesn't-Move]], [[Episode 6 — First Real Wesley Suspicion]], [[Episode 7 — The Breather]], [[Episode 8 — Wesley Confirmed]], [[The Equinox Arc]].

### [[Bestiary]]
[[The Hush]], [[The Mercer Mirror]], [[The Storm-That-Doesn't-Move]].

### [[Player-Facing Materials]] — safe to show players
[[Player Sourcebook]], [[Character Creation Guide]], [[Dungeoneering Rules]], [[Negotiation Rules Reference]], [[The Campaign Pitch]], [[Pre-Generated Characters]].

### [[DM-OS & Prep]] — the operating system behind the table
[[DM-OS — Architectural Principles]], [[DM-OS — v0.1 Specification]], [[DM-OS — Pre-Campaign Timeline]], [[Archivist AI Documentation]], [[Next Prep Targets]], [[Outstanding Design Questions]].

### Meta
[[Vault Setup & Conventions]] · [[Source Documents]] · [[MCP Connector Handoff]]

## Live & next

- **Immediate prep** is tracked in [[Next Prep Targets]] (Tiers 1–7) and open threads in [[Outstanding Design Questions]].
- Many entities are **locked after PC creation** (Sam, residences, the Rivals' final motivations/pitfalls, the gear stash). Those notes are marked `status: tbd` or `provisional`.
- This vault is the integrated layer over the source documents in [[Source Documents]]; the originals remain the authoritative long-form record, and each note's `source` property points back to them.

## Conventions in one breath

A note earns its place when it's an **atomic, linkable entity** — an NPC, location, faction, system, plot thread, episode, or piece of lore. Notes summarize and interlink; they point to [[Source Documents]] for exhaustive detail rather than duplicating it. Title Case names; `Up:` to the domain MOC; DM-secrets flagged. See [[Vault Setup & Conventions]] for the full ruleset and the planned [[MCP Connector Handoff]] for how an LLM will read and update this vault.